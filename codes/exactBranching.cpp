...
int b_branching(Graph& G, const Graph& G_orig,int max_obj, vector <edge>& sol, vector<vector<int>>& uv){
    rec_cnt ++;
    ...
    FOR (i,0,uv.size()) {
        ...
        G.flip_edge(u,v);
        best = b_branching(G, G_orig, max_obj-w, best_sol, uv);
        if(best != -1) {
            best += w;
        ...
        if(merge_flag){
            ...
            int cost = b_branching(G, G_orig, max_obj-mergecost, tmpsol, uv);
...
int bc_branching(Graph& G, const Graph& G_orig,int max_obj, vector <edge>& sol, int rec_depth, vector<vector<int>>& uv){
    rec_cnt ++;
    ...
    FOR (i,0,uv.size()) {
        ...
        G.forbid(u, v, best_sol, G_orig);
        G.flip_edge(u,v);
        best = bc_branching(G, G_orig, max_obj-w, best_sol, rec_depth+1, uv);
        if(best != -1) {
            best += w;
            if(best < max_obj) max_obj = best;
        }
        ...
        if(merge_flag){
            ...
            MergeData mg(G_orig.num_nodes);
            int mergecost = G.merge_nodes(G.name_to_ind[u_name], G.name_to_ind[v_name], tmpsol, mg, G_orig);
            int cost = bc_branching(G, G_orig, max_obj-mergecost, tmpsol, rec_depth, uv);
            if((cost != -1) && (best == -1 || cost + mergecost < best)){
                best = cost + mergecost;
                best_sol = tmpsol;
            }
    ...
    FOR (i,0,uv.size()) {
        ...
        G.forbid(u, v, best_sol, G_orig);
        G.flip_edge(u,v);
        best = bc_branching(G, G_orig, max_obj-w, best_sol, rec_depth, uv);
        if(best != -1) {
            best += w;
            if(best < max_obj) max_obj = best;
        }
...
int default_branching(Graph& G, const Graph& G_orig,int max_obj, vector <edge>& sol, vector<vector<int>>& uv){
    rec_cnt ++;
    ...
    FOR (i,0,uv.size()) {
        ...
        G.forbid(u, v, best_sol, G_orig);
        G.flip_edge(u,v);
        best = default_branching(G, G_orig, max_obj-w, best_sol, uv);
        if(best != -1) {
            best += w;
            if(best < max_obj) max_obj = best;
        }
        ...
        if(merge_flag){
            ...
            MergeData mg(G_orig.num_nodes);
            int mergecost = G.merge_nodes(G.name_to_ind[u_name], G.name_to_ind[v_name], tmpsol, mg, G_orig);
            int cost = default_branching(G, G_orig, max_obj-mergecost, tmpsol, uv);
            if((cost != -1) && (best == -1 || cost + mergecost < best)){
                best = cost + mergecost;
                best_sol = tmpsol;
            }
...
int c_branching(Graph& G, const Graph& G_orig,int max_obj, vector <edge>& sol, int rec_depth, vector<vector<int>>& uv){
    rec_cnt ++;
    ...
    FOR (i,0,uv.size()) {
        ...
        G.forbid(u, v, best_sol, G_orig);
        G.flip_edge(u,v);
        best = c_branching(G, G_orig, max_obj-w, best_sol, rec_depth+1, uv);
        if(best != -1) {
            best += w;
            if(best < max_obj) max_obj = best;
        }
        ...
        if(merge_flag){
            ...
            MergeData mg(G_orig.num_nodes);
            int mergecost = G.merge_nodes(G.name_to_ind[u_name], G.name_to_ind[v_name], tmpsol, mg, G_orig);
            int cost = c_branching(G, G_orig, max_obj-mergecost, tmpsol, rec_depth+1, uv);
            if((cost != -1) && (best == -1 || cost + mergecost < best)){
                best = cost + mergecost;
                best_sol = tmpsol;
            }
